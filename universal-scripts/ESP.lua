...
local Window = library:AddWindow("Universal Drawing ESP", {
    main_color = Color3.fromRGB(91, 46, 134),
    min_size = Vector2.new(300, 460),
...
local masterSwitch = Home:AddSwitch("Master Switch", function(bool)
    ESP:Toggle(bool)
end); masterSwitch:Set(true)

local setDistance = 5000; local DistanceSlider = Home:AddSlider("Max Viewdistance", function(x)
    ESP.MaxDistance = x 
    setDistance = x 
end, {
    ["min"] = 0, 
...
local DistanceSwitch = Home:AddSwitch("Unlimited Distance", function(bool)
    if bool == true then 
        ESP.MaxDistance = math.huge 
    else 
        ESP.MaxDistance = setDistance 
    end 
end); DistanceSwitch:Set(true); ESP.MaxDistance = math.huge -- Precaution from config loading 

-- Non affected block 
Home:AddSwitch("Show Players", function(bool)
    ESP.Players = bool
end); local teamSwitch = Home:AddSwitch("Show Teammates", function(bool)
    ESP.TeamMates  = bool 
end); teamSwitch:Set(true); Home:AddSwitch("Show Boxes", function(bool)
    ESP.Boxes = bool
end); Home:AddSwitch("Show Names", function(bool)
    ESP.Names = bool
end); Home:AddSwitch("Show Tracers", function(bool)
    ESP.Tracers = bool
end); Home:AddSwitch("Show Health", function(bool)
    ESP.Health = bool
end)

local teamColor = Home:AddSwitch("Team Color", function(bool)
    ESP.TeamColor = bool
end); teamColor:Set(true)

Home:AddLabel("Custom Color")

local ColorPicker = Home:AddColorPicker(function(color)
    ESP.Color = color 
end); ColorPicker:Set(Color3.new(1, 1, 1))
...
-- Find asset 
if not isfile('502Development.png') then
    writefile('502Development.png', game:HttpGet('https://i.imgur.com/JXS5V2C.png'))
end

game.StarterGui:SetCore("SendNotification", {
	Title = "502Development";
	Text = "This script was created by 502Development.";
	Icon = getsynasset('502Development.png');
	Duration = 5;
})
