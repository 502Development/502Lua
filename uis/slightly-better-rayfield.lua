...
function Hide()
	Debounce = true
	RayfieldLibrary:Notify("Interface Hidden","The Rayfield interface has been hidden, you can unhide the interface by tapping End on your keyboard.")
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			UserInputService.InputBegan:Connect(function(input, processed)
				if CheckingForKey then
					if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Enum.KeyCode.End then
...
UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode.End and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
